/*
 * Graphics backend for Equinox.
 * This is a patched version of the default Louvre Wayland backend, tweaked for our needs.
 * Original Author: Eduardo Hopperdietzel (ehopperdietzel)
 *
...
public:
    ...
    {
        return LGraphicBackendWayland; // we're pretending this so that the default wayland input backend works
    }
    ...
    static void createWindow()
    {
        ...
        xdg_toplevel_set_app_id(xdgToplevel, "io.github.equinoxhq.Equinox");
        xdg_toplevel_set_title(xdgToplevel, "Equinox");
    ...
    {
        return "The EquinoxHQ Team";
    }

    static const char *outputGetModelName(LOutput */*output*/)
    {
        return "Equinox Wayland Backend";
    }

    static const char *outputGetDescription(LOutput */*output*/)
    {
        return "Louvre compositor running on a Wayland EGL window with EquinoxHQ's patches";
    }
	...
	xdg_toplevel_set_fullscreen(xdgToplevel, surfaceOutputs[surfaceOutputs.size() - 1]); // We want to fullscreen on all outputs
        updateSurfaceScale();
    }
